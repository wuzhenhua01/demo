...
- **MUST NOT** 通过 `brew install` 安装任何软件。
- **MUST NOT** 使用 Gradle/Maven Wrapper，**MUST** 使用上表中的本地 Gradle/Maven。
- **MUST NOT** 在项目级配置文件中声明 repositories，仓库 **MUST** 由全局配置统一管理。
- 命令与脚本 **MUST** 使用 `bash` 语法。
- 联网下载先直连，失败 3 次后使用[网络代理](http://127.0.0.1:7897)；代理再失败 3 次，**MUST** 停止等待用户操作，**MUST NOT** 使用任何 workaround。
